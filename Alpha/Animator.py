...
#imports ifc
path = os.path.abspath("D:/Program Files (x86)/Blender/2.90/scripts/BlenderShellDev/Alpha/blend/shedule_Revision2stair-Final.ifc")
ifc = ifcopenshell.open(path)
#selector = Selector()
#Extract IfcRelAssignsToProcess
RelAssign = ifc.by_type('IfcRelAssignsToProcess')
#print(RelAssign[0],len(RelAssign))
...
def getProcessItem():
    ...
    for i in range(len(RelAssign)):
        ...
        #print('RelItems',RelItems,len(RelItems))
        # convert to ifcopenshell.entity_instance.entity_instance
        GuID_list = []

        for j in range(len(RelItems)):
            temp_item = RelItems[j]
            print('RelItems', temp_item, len(temp_item))
            for k in range(len(temp_item)):
                print(k,'_',temp_item[k])
            temp_id = str(temp_item[2])
            tag = re.search(r"\[(\w+)\]", temp_id)
            print('tag',tag.group(1))
            GuID_list.append(tag.group(1))
